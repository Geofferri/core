...
bool RTSCAction::Execute(Event& event)
{
	...
	else if (command.find("save here ") != std::string::npos)
	{
		...
		if (Creature* wpCreature = bot->SummonCreature(15631, spellPosition.getX(), spellPosition.getY(), spellPosition.getZ(), spellPosition.getO(), TEMPSUMMON_TIMED_DESPAWN, 2000.0f))
                                {
